...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody AuthRequest request) {
        try {
            ...
            // ✅ FIX: fetch user from DB to get name + isVerified for the response
            UserEntity user = userRepository.findByEmail(request.getEmail())
                    .orElseThrow(() -> new RuntimeException("User not found"));
            ...
            // ✅ FIX: AuthResponse now includes name + isVerified
            // Frontend Auth.tsx does: loginUser({ name: res.name, email: res.email, isVerified: res.isVerified })
            return ResponseEntity.ok()
                    ...
                    .body(new AuthResponse(
                            ...
                            jwtToken,
                            user.getName(),
                            user.getIsAccountVerified()   // maps to isVerified in frontend
                    ));
    ...
    // ✅ FIX: logout endpoint — clears the JWT cookie so frontend use-auth works
    @PostMapping("/logout")
