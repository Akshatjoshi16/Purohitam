...
import {
  ...
  BookOpen, ChevronDown, ChevronUp, Search, RefreshCw,
  Phone, Mail, MapPin, Calendar, Loader2, AlertTriangle,
  Users, TrendingUp, Filter, LogOut, X,
} from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/hooks/use-auth";
import {
...
// ─── Status config ────────────────────────────────────────────────
const STATUS_CFG: Record<BookingStatus, {
  label: string; textColor: string; bgColor: string; borderColor: string; dotColor: string; icon: React.ElementType;
}> = {
  PENDING:   { label: "Pending",   textColor: "text-amber-700",   bgColor: "bg-amber-50",   borderColor: "border-amber-200",   dotColor: "bg-amber-400",   icon: Clock        },
  CONFIRMED: { label: "Confirmed", textColor: "text-emerald-700", bgColor: "bg-emerald-50", borderColor: "border-emerald-200", dotColor: "bg-emerald-500", icon: CheckCircle2  },
  COMPLETED: { label: "Completed", textColor: "text-blue-700",    bgColor: "bg-blue-50",    borderColor: "border-blue-200",    dotColor: "bg-blue-500",    icon: Trophy        },
  CANCELLED: { label: "Cancelled", textColor: "text-red-600",     bgColor: "bg-red-50",     borderColor: "border-red-200",     dotColor: "bg-red-400",     icon: X             },
};
...
const StatusBadge = ({ status }: { status: BookingStatus }) => {
  ...
  return (
    <span className={cn(
      ...
      c.textColor, c.bgColor, c.borderColor
    )}>
...
const StatCard = ({ label, value, icon: Icon, iconClass, onClick, active }: {
  label: string; value: number; icon: React.ElementType;
  iconClass: string; onClick?: () => void; active?: boolean;
}) => (
  <button
    onClick={onClick}
    className={cn(
      "w-full text-left bg-white rounded-2xl p-5 border transition-all duration-150",
      active ? "border-orange-300 shadow-md shadow-orange-100 ring-1 ring-orange-200"
  ...
  >
    <div className="flex items-start justify-between mb-3">
      ...
      {active && (
        <span className="text-[10px] font-medium text-orange-500 bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-full">
          Active
        </span>
      )}
    </div>
...
const ActionModal = ({ booking, onClose, onConfirm, loading }: {
  ...
  const actions: {
    status: BookingStatus; label: string; desc: string;
    activeClass: string; idleClass: string;
  }[] = [
    {
      status: "CONFIRMED",
      label: "✅  Confirm Booking",
      desc: "Devotee receives a confirmation email with payment instructions.",
      activeClass: "border-emerald-400 bg-emerald-50 text-emerald-800",
      idleClass: "border-stone-200 hover:border-emerald-300 hover:bg-emerald-50/60 text-stone-700",
    },
    {
      status: "COMPLETED",
      label: "🏆  Mark as Completed",
      desc: "Marks the ritual as done. Devotee gets a completion email.",
      activeClass: "border-blue-400 bg-blue-50 text-blue-800",
      idleClass: "border-stone-200 hover:border-blue-300 hover:bg-blue-50/60 text-stone-700",
    },
    {
      status: "CANCELLED",
      label: "❌  Cancel / Reject",
      desc: "Booking is cancelled. Devotee is notified with your reason.",
      activeClass: "border-red-400 bg-red-50 text-red-800",
      idleClass: "border-stone-200 hover:border-red-300 hover:bg-red-50/60 text-stone-700",
    },
  ].filter(a => a.status !== booking.status);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <p className="text-xs text-stone-400 mb-1">Booking #{booking.id}</p>
              <h3 className="text-base font-bold text-stone-800">{booking.poojaName}</h3>
              <p className="text-sm text-stone-500 mt-0.5">{booking.devoteeName}</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <StatusBadge status={booking.status} />
              <button onClick={onClose}
          ...
          {/* Quick info */}
          <div className="grid grid-cols-2 gap-2 mb-5 p-3 bg-stone-50 rounded-xl border border-stone-100 text-xs text-stone-500">
            <span className="flex items-center gap-1.5"><Calendar className="w-3 h-3 text-orange-400" />{booking.ritualDate}</span>
            <span className="flex items-center gap-1.5"><Clock className="w-3 h-3 text-orange-400" />{booking.timeSlot}</span>
            <span className="flex items-center gap-1.5 truncate col-span-1"><MapPin className="w-3 h-3 text-orange-400 flex-shrink-0" /><span className="truncate">{booking.location}</span></span>
            <span className="flex items-center gap-1.5"><Phone className="w-3 h-3 text-orange-400" />{booking.devoteePhone}</span>
          </div>

          {/* Action selector */}
          <p className="text-[11px] font-semibold uppercase tracking-wider text-stone-400 mb-2">Select Action</p>
          <div className="space-y-2 mb-4">
            {actions.map(a => (
              <button
                ...
                className={cn(
                  "w-full px-4 py-3 rounded-xl border text-left transition-all",
                  selectedStatus === a.status ? a.activeClass : a.idleClass
                )}
              >
          ...
          <div className="mb-5">
            <label className="text-[11px] font-semibold uppercase tracking-wider text-stone-400 mb-1.5 block">
              Message to Devotee
              <span className="text-stone-300 font-normal normal-case tracking-normal ml-1">(shown in app + email)</span>
            </label>
            <textarea
              value={notes}
              onChange={e => setNotes(e.target.value)}
              rows={3}
              placeholder={
                selectedStatus === "CONFIRMED" ? "e.g. Pandit Sharma will contact you the evening before..."
                : selectedStatus === "CANCELLED" ? "e.g. Unavailable on this date, please rebook..."
                : "Optional note for the devotee..."
              }
              className="w-full bg-stone-50 border border-stone-200 rounded-xl px-3.5 py-2.5 text-sm text-stone-700 placeholder:text-stone-300 focus:outline-none focus:border-orange-300 focus:ring-2 focus:ring-orange-100 resize-none transition-all"
            />
          </div>
          ...
          <div className="flex gap-2.5">
            ...
            <button
              onClick={() => selectedStatus && onConfirm(selectedStatus, notes)}
              disabled={!selectedStatus || loading}
              className="flex-1 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:bg-stone-100 disabled:text-stone-400 text-white text-sm font-semibold transition-colors flex items-center justify-center gap-2"
            >
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" />Saving...</> : "Apply Action"}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3 px-4 py-3.5">
        <div className={cn("w-2 h-2 rounded-full flex-shrink-0", STATUS_CFG[booking.status].dotColor)} />

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-sm text-stone-800 truncate">{booking.poojaName}</span>
            <span className="text-stone-300 hidden sm:block">·</span>
            <span className="text-xs text-stone-500 hidden sm:block truncate">{booking.devoteeName}</span>
          </div>
          <div className="flex items-center gap-3 mt-0.5 text-xs text-stone-400">
            <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{fmtDate(booking.ritualDate)}</span>
            <span className="hidden sm:flex items-center gap-1">
              <MapPin className="w-3 h-3" />
              <span className="truncate max-w-[90px]">{booking.location}</span>
            </span>
            <span className="hidden md:block">Booked {fmtCreated(booking.createdAt)}</span>
          </div>
        </div>

        <span className="text-sm font-semibold text-orange-600 hidden sm:block flex-shrink-0">
          {booking.poojaPrice}
        </span>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            ...
            className="text-xs font-medium bg-orange-50 hover:bg-orange-100 border border-orange-200 text-orange-600 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
          >
            Manage
          </button>
          <button
      ...
      {/* Expanded detail */}
      <div className={cn("overflow-hidden transition-all duration-200", expanded ? "max-h-64" : "max-h-0")}>
        <div className="px-4 pb-4 pt-3 border-t border-stone-100 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Phone",     val: booking.devoteePhone, icon: Phone    },
            { label: "Email",     val: booking.devoteeEmail, icon: Mail     },
            { label: "Time Slot", val: booking.timeSlot,     icon: Clock    },
            { label: "Location",  val: booking.location,     icon: MapPin   },
          ].map(({ label, val, icon: Icon }) => (
            <div key={label}>
              ...
              <p className="text-xs text-stone-600 flex items-start gap-1">
                <Icon className="w-3 h-3 text-orange-400 flex-shrink-0 mt-0.5" />
          ...
          {booking.specialInstructions && (
            <div className="col-span-2">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-stone-400 mb-1">Special Instructions</p>
              <p className="text-xs text-stone-600">{booking.specialInstructions}</p>
            </div>
          )}
          {booking.adminNotes && (
            <div className="col-span-2 md:col-span-4">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-orange-500 mb-1">Admin Note</p>
...
const AdminDashboard = () => {
  ...
  const handleStatusUpdate = async (status: BookingStatus, notes: string) => {
    ...
    try {
      ...
      const s = await getBookingStats();
      setStats(s);
      toast.success("Booking updated! Email sent to devotee.", {
        description: `${modalBooking.poojaName} → ${status.toLowerCase()}.`,
      });
      setModalBooking(null);
    } catch (err: any) {
      toast.error("Update failed", { description: err.message });
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex">
      ...
      {/* ── SIDEBAR ─────────────────────────────────────────── */}
      <aside className="w-56 flex-shrink-0 bg-white border-r border-stone-200 hidden md:flex flex-col sticky top-0 h-screen">
        ...
        <div className="px-5 py-5 border-b border-stone-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-bold text-stone-800 tracking-wide">PUROHITAM</p>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {([
            ...
            >
              ...
              {tab.label}
              {tab.id === "bookings" && pendingCount > 0 && (
                <span className="ml-auto bg-amber-400 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">
                  {pendingCount}
                </span>
              )}
            </button>
        ...
        <div className="px-3 py-4 border-t border-stone-100">
          <div className="flex items-center gap-2.5 px-3 mb-3">
            <div className="w-7 h-7 rounded-full bg-orange-100 border border-orange-200 flex items-center justify-center text-orange-600 font-bold text-xs flex-shrink-0">A</div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-stone-700">Admin</p>
              <p className="text-[10px] text-stone-400 truncate">{user?.email}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-stone-500 hover:bg-red-50 hover:text-red-600 transition-colors"
          >
        ...
        <header className="bg-white border-b border-stone-200 sticky top-0 z-30 flex-shrink-0">
          <div className="flex items-center justify-between px-6 h-14">
            ...
            <div className="md:hidden flex items-center gap-2">
              <span className="text-orange-500 font-bold text-sm">🔱 PUROHITAM</span>
            </div>

            {/* Desktop: page title */}
            <h1 className="hidden md:block text-sm font-semibold text-stone-600">
              {activeTab === "overview" ? "Dashboard Overview" : "Booking Management"}
            </h1>

            <div className="flex items-center gap-2">
              {pendingCount > 0 && (
                <button
                  onClick={() => { setActiveTab("bookings"); setFilterStatus("PENDING"); }}
                  className="flex items-center gap-1.5 text-xs font-medium bg-amber-50 border border-amber-200 text-amber-700 px-3 py-1.5 rounded-full hover:bg-amber-100 transition-colors"
                >
                  <AlertTriangle className="w-3.5 h-3.5" />
              ...
              <button
                ...
                disabled={refreshing}
                className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-stone-700 border border-stone-200 hover:border-stone-300 px-3 py-1.5 rounded-full transition-all"
              >
                <RefreshCw className={cn("w-3.5 h-3.5", refreshing && "animate-spin")} />
                <span className="hidden sm:block">Refresh</span>
              </button>
              <button onClick={handleLogout} className="md:hidden text-stone-400 hover:text-red-500 transition-colors p-1.5">
                <LogOut className="w-4 h-4" />
              </button>
            </div>
          </div>
          ...
          <div className="md:hidden flex border-t border-stone-100">
            {([
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={cn(
                  "flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-medium border-b-2 transition-all",
                  activeTab === tab.id ? "border-orange-400 text-orange-600" : "border-transparent text-stone-400"
                )}>
                <tab.icon className="w-3.5 h-3.5" />
                {tab.label}
                {tab.id === "bookings" && pendingCount > 0 && (
                  <span className="bg-amber-400 text-white text-[9px] font-bold px-1.5 rounded-full">{pendingCount}</span>
                )}
              </button>
            ))}
          </div>
          ...
          {activeTab === "overview" && (
            <div className="space-y-6" style={{ animation: "fadeUp 0.3s ease both" }}>
              ...
              {dataLoading ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                  <StatCard label="Total"     value={stats.total}     icon={Users}        iconClass="text-stone-500"   />
                  <StatCard label="Pending"   value={stats.pending}   icon={Clock}        iconClass="text-amber-500"
                    onClick={() => { setActiveTab("bookings"); setFilterStatus("PENDING"); }}
                    active={activeTab === "bookings" && filterStatus === "PENDING"} />
                  <StatCard label="Confirmed" value={stats.confirmed} icon={CheckCircle2} iconClass="text-emerald-500"
                    onClick={() => { setActiveTab("bookings"); setFilterStatus("CONFIRMED"); }} />
                  <StatCard label="Completed" value={stats.completed} icon={Trophy}       iconClass="text-blue-500"
                    onClick={() => { setActiveTab("bookings"); setFilterStatus("COMPLETED"); }} />
                  <StatCard label="Cancelled" value={stats.cancelled} icon={XCircle}      iconClass="text-red-400"
                    onClick={() => { setActiveTab("bookings"); setFilterStatus("CANCELLED"); }} />
                </div>
              )}
              ...
              {stats && stats.total > 0 && (
                <div className="bg-white rounded-2xl border border-stone-200 p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-stone-400 mb-1">Completion Rate</p>
                      <p className="text-2xl font-bold text-stone-800">
                        {Math.round(((stats.confirmed + stats.completed) / stats.total) * 100)}%
                      </p>
                      <p className="text-xs text-stone-400 mt-0.5">confirmed + completed out of all bookings</p>
                    </div>
                    <TrendingUp className="w-7 h-7 text-stone-300" />
                  </div>
                  <div className="flex h-2 rounded-full overflow-hidden bg-stone-100 gap-0.5">
                    {[
                      { val: stats.confirmed, color: "bg-emerald-400" },
                      { val: stats.completed, color: "bg-blue-400"    },
                      { val: stats.pending,   color: "bg-amber-400"   },
                      { val: stats.cancelled, color: "bg-red-300"     },
                    ].filter(x => x.val > 0).map((x, i) => (
                      <div key={i} className={cn("h-full", x.color)} style={{ width: `${(x.val / stats.total) * 100}%` }} />
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-4 mt-3">
                    {[
                      ...
                      { label: "Completed", color: "bg-blue-400",    val: stats.completed },
                      { label: "Pending",   color: "bg-amber-400",   val: stats.pending   },
                      { label: "Cancelled", color: "bg-red-300",     val: stats.cancelled },
                    ].map(x => (
                      <div key={x.label} className="flex items-center gap-1.5">
                        <div className={cn("w-2 h-2 rounded-full", x.color)} />
                        <span className="text-xs text-stone-500">
                          {x.label} <span className="font-semibold text-stone-700">({x.val})</span>
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold text-stone-700">Needs Your Attention</h2>
                      <span className="bg-amber-100 border border-amber-200 text-amber-700 text-xs font-semibold px-2 py-0.5 rounded-full">
              ...
              {!dataLoading && bookings.length === 0 && (
                <div className="bg-white rounded-2xl border border-stone-200 py-20 text-center">
                  ...
                  <p className="text-stone-400 text-sm font-medium">No bookings yet</p>
                  <p className="text-stone-300 text-xs mt-1">Bookings will appear here as devotees make requests</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-4" style={{ animation: "fadeUp 0.3s ease both" }}>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search by name, pooja, email or phone..."
                    value={search}
                    onChange={e => setSearch(e.target.value)}
                    className="w-full h-10 pl-10 pr-4 bg-white border border-stone-200 rounded-xl text-sm text-stone-700 placeholder:text-stone-400 focus:outline-none focus:border-orange-300 focus:ring-2 focus:ring-orange-100 transition-all"
                  />
                </div>
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                  ...
                  {(["ALL", "PENDING", "CONFIRMED", "COMPLETED", "CANCELLED"] as FilterStatus[]).map(f => (
                    <button
                      ...
                      className={cn(
                        "text-xs font-medium px-3 py-1.5 rounded-full border whitespace-nowrap transition-all",
                        filterStatus === f
              ...
              <p className="text-xs text-stone-400">
                Showing <span className="font-semibold text-stone-600">{filtered.length}</span> booking{filtered.length !== 1 ? "s" : ""}
                {search && <> for "<span className="font-medium text-stone-700">{search}</span>"</>}
              </p>
              ...
              {!dataLoading && (
                <div className="space-y-2">
                  {filtered.map((b, i) => (
                    <div key={b.id} style={{ animationDelay: `${i * 25}ms`, animation: "fadeUp 0.3s ease both" }}>
                      <BookingRow booking={b} onAction={setModalBooking} />
                    </div>
                  ))}
                </div>
