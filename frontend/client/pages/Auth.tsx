...
import { useAuth } from "@/hooks/use-auth";
import { login as loginApi, signup as signupApi } from "@/services/api";
import { ADMIN_EMAIL } from "@/App"; // ← import the admin email constant
...
const OrnamentalDivider = () => (
  <div className="flex items-center gap-3 my-6">
    <div className="h-px flex-1 bg-stone-200" />
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      <path d="M10 2 L12 8 L18 8 L13 12 L15 18 L10 14 L5 18 L7 12 L2 8 L8 8 Z" fill="#f97316" opacity="0.5" />
    </svg>
    <div className="h-px flex-1 bg-stone-200" />
  </div>
);
...
/* ─── Icon input ─────────────────────────────────── */
const IconInput = ({
  icon: Icon, rightIcon, onRightClick, ...props
}: {
  icon: React.ElementType;
  rightIcon?: React.ElementType;
  onRightClick?: () => void;
  ...
  <div className="relative group">
    ...
    {rightIcon && onRightClick && (
      <button
        type="button" onClick={onRightClick}
        className="absolute right-4 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 transition-colors"
        tabIndex={-1}
      >
        {React.createElement(rightIcon, { className: "w-4 h-4" })}
...
const Auth = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Admin goes to admin panel, regular user goes to home
  ...
  return (
    <div className="min-h-screen flex">
      ...
      {/* LEFT PANEL */}
      <div className="hidden lg:flex lg:w-[52%] relative overflow-hidden flex-col">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/mahakallok.jpeg')" }} />
        <div className="absolute inset-0 bg-gradient-to-br from-black/80 via-stone-900/70 to-orange-950/60" />
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" aria-hidden>
          <span className="text-[22rem] font-cinzel font-bold text-white/[0.03] leading-none">ॐ</span>
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full px-16 py-14">
          ...
          <div className="flex-1 flex flex-col justify-center max-w-md">
            ...
            <div className="border-l-2 border-orange-500/40 pl-5">
              <p className="text-orange-300/80 italic text-sm mb-1">"श्रद्धावान् लभते ज्ञानम्"</p>
              <p className="text-orange-200/40 text-xs">The faithful one obtains wisdom. — Bhagavad Gita 4.39</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-stone-50 px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-8">
            <Link to="/"><span className="text-orange-600 font-cinzel font-bold text-2xl tracking-wider">PUROHITAM</span></Link>
            <p className="text-stone-400 text-xs mt-1 tracking-widest">UJJAIN · वैदिक सेवाएँ</p>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-stone-200/80 shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              ...
              {tab === "login" && (
                <form onSubmit={handleLogin} className="space-y-4">
                  <div>
                    <label className="text-[11px] text-stone-500 uppercase tracking-widest mb-1.5 block">Email address</label>
                    <IconInput icon={Mail} type="email" placeholder="you@example.com" required
                      value={loginData.email} onChange={(e) => setLoginData({ ...loginData, email: e.target.value })} />
                  </div>
                  <div>
                    <label className="text-[11px] text-stone-500 uppercase tracking-widest mb-1.5 block">Password</label>
                    <IconInput icon={Lock} type={showLoginPw ? "text" : "password"} placeholder="••••••••" required
                      value={loginData.password} onChange={(e) => setLoginData({ ...loginData, password: e.target.value })}
                      rightIcon={showLoginPw ? EyeOff : Eye} onRightClick={() => setShowLoginPw(!showLoginPw)} />
                  </div>
                  <OrnamentalDivider />
                  ...
                  <p className="text-center text-xs text-stone-400 mt-2">
                    Don't have an account?{" "}
                    <button type="button" onClick={() => setTab("register")} className="text-orange-500 hover:text-orange-700 font-medium transition-colors">
                      Register here
                    </button>
                  </p>
                </form>
              ...
              {tab === "register" && (
                <form onSubmit={handleRegister} className="space-y-4">
                  <div>
                    <label className="text-[11px] text-stone-500 uppercase tracking-widest mb-1.5 block">Full name</label>
                    <IconInput icon={User} type="text" placeholder="Your full name" required
                  ...
                  <div>
                    <label className="text-[11px] text-stone-500 uppercase tracking-widest mb-1.5 block">Password</label>
                    <IconInput icon={Lock} type={showSignupPw ? "text" : "password"} placeholder="Create a strong password" required
                      value={signupData.password} onChange={(e) => setSignupData({ ...signupData, password: e.target.value })}
                      rightIcon={showSignupPw ? EyeOff : Eye} onRightClick={() => setShowSignupPw(!showSignupPw)} />
                  </div>
                  <p className="text-[11px] text-stone-400 -mt-1 flex items-center gap-1">
                    <span className="w-1 h-1 rounded-full bg-stone-300 inline-block" />
                    Use at least 8 characters with a mix of letters and numbers
                  ...
                  <p className="text-center text-xs text-stone-400 mt-2">
                    Already have an account?{" "}
                    <button type="button" onClick={() => setTab("login")} className="text-orange-500 hover:text-orange-700 font-medium transition-colors">
                      Sign in
                    </button>
                  </p>
                </form>
              ...
              <div className="mt-8 pt-6 border-t border-stone-100 flex items-center justify-center gap-6">
                {[{ icon: "🔒", label: "Secure login" }, { icon: "🛡️", label: "Data protected" }, { icon: "✨", label: "Verified priests" }].map((b, i) => (
                  <div key={i} className="flex flex-col items-center gap-1">
                    <span style={{ fontSize: 16 }}>{b.icon}</span>
                    <span className="text-[10px] text-stone-400 tracking-wide">{b.label}</span>
                  </div>
          ...
          <div className="mt-6 text-center">
            <Link to="/" className="text-stone-400 text-xs hover:text-orange-500 transition-colors flex items-center justify-center gap-1">
              ← Return to home
            </Link>
          </div>
          <p className="mt-4 text-center text-stone-300 text-[11px] italic tracking-wider">श्रद्धा–सेवा–संयोगः</p>
        </div>
      </div>
    </div>
  );
